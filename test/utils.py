...
parser.add_argument('-dev', '--development', action='store_const', const=True, default=False,
					help='Toggle experimental scraping to target: Kaltura player.(Default = False)')
	...
	try:
		...
		if type(results) is list and len(results) > 0:
			...
			for downloadable in results:
				...
				# base_directory = os.path.join(chapter_dir, f'{base_name}'+'{}')
				# file paths
		...
		# for l in download_workers:
		# 	if list(l.values())[0].result():
		# 		break
